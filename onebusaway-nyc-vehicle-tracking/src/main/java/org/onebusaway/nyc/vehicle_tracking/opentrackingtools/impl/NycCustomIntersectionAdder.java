...
/**
 * Taken from JTS's IntersectionAdder.  Modified to only add proper (interior) intersections.
 * 
 * @version 1.7
 */
    ...
    public void merge(SegmentString mergeTarget, SegmentString ssToMerge,
      ...
      for (final NycTrackingGraph.SegmentInfo si : (List<NycTrackingGraph.SegmentInfo>) ssToMerge.getData()) {
        newSegmentInfos.add(new NycTrackingGraph.SegmentInfo(si.getShapeId(), 
            si.getGeomNum(),
            si.getIsSameOrientation().equals(new Boolean(isSameOrientation))));
      }
    ...
    if (li.hasIntersection()) {
      ...
      if (!isTrivialIntersection(e0, segIndex0, e1, segIndex1)) {
        ...
        if (li.isInteriorIntersection() && Angle.diff(sAngle1, sAngle2) < 1e-4) {
          /*
           * the segments aren't equal, but overlap, and are in the same direction.
           */
          ...
          /*
           * if the start points are the same but the ends are not, we check for a split along
           * the same prior path.  we confirm the same prior path by matching angles of the
           * segments leading to the split point.  
           */
          Coordinate p00b = e0.getCoordinates()[segIndex0 - 1];
          Coordinate p10b = e1.getCoordinates()[segIndex1 - 1];
