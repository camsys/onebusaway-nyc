...
/**
 * HTTP Proxy Servet.  Listens for HTTP Posts and blindly
 * throws the content of the post onto the configured topic queue.
 *
 ...
 * <li>queue_host: the interface to bind to (e.g., * to listen on all interfaces)
...
public class BHSListenerServlet extends HttpServlet {
    ...
    public synchronized void init() throws ServletException {
        IPublisher publisher = (IPublisher)getServletConfig().getServletContext().getAttribute(PUBLISHER_KEY);
        ...
        if (publisher == null) {
            ...
            publisher.open(protocol, host, port); //todo pull this out to config param
            // lazy instantiate and save with application context
            getServletConfig().getServletContext().setAttribute(PUBLISHER_KEY, publisher);
        }
    }
    ...
    private void service(ServletInputStream stream) throws IOException {
        ...
        /* 
         * messages are expected to be small so in-memory processing is not
         * an issue.
         */
        while (noData != -1) {
    ...
    /**
     * convenience method to retrieve and init parameter or return the 
    ...
    /**
     * convenience method to retrieve and init parameter or return the 
     * default value otherwise.  This method overrides based on type.
     */
