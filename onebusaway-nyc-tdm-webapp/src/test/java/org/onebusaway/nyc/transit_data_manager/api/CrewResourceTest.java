...
public class CrewResourceTest extends ResourceTest {
  ...
  public void testPaddedRunIds() throws Exception {
    ...
    for (SCHOperatorAssignment assignment : list) {
      ...
      if (runId.contains("-0")) {
        _log.error("found runId=" + runId + " for assignment="
            + assignment.toString());
      } else {
        // _log.info("found runId=" + runId + " for assignment=" +
        // assignment.toString());
      }
  ...
  public void testDuplicateAssignments() throws Exception {
    HashMap<String, OperatorAssignment> passMap = testDuplicateAssignments(
        "CIS_20120727.txt", "2012-07-27");
    ...
    assertNotNull(test2);
    assertEquals("Q4420", test2.getRunRoute()); // duplicate CAST should be
                                                // excluded
  }
  ...
  public void testNumericPassNumbers() throws Exception {
    ...
    List<OperatorAssignment> jsonOpAssigns = new UTSUtil().listConvertOpAssignTcipToJson(
        tcipToJsonConverter,
  ...
  public void testMiscRunIds() throws Exception {
    ...
    UtsCrewAssignsToDataCreator process = new UtsCrewAssignsToDataCreator(
        tmpInFile);
    DepotIdTranslator translator = new TestDepotIdTranslator();
    process.setDepotIdTranslator(translator);
    // no need to set depotIdTranslater
    OperatorAssignmentData data = process.generateDataObject();
    assertNotNull(data);
    DateMidnight serviceDate = new DateMidnight("2012-07-27");
    ...
    for (OperatorAssignment oa : jsonOpAssigns) {
      ...
      if ("MISC".equals(runRoute)) {
        ...
        /*
         * we are expecting MISC-YU-101, but occasionally we get MISC-YU-OTOVL
         */
        if (!runId.matches("^MISC-[A-Z][A-Z]-(\\d{3}|[A-Z0-9a-z]{4,5})$")) {
  ...
  public HashMap<String, OperatorAssignment> testDuplicateAssignments(
      String filename, String serviceDateStr) throws Exception {
  ...
  public class TestDepotIdTranslator extends DepotIdTranslator {

    @Override
    public String getMappedId(String dataSourceId, String fromId) {
      if (fromId != null) {
        return fromId.substring(0, 2);
      }
      return fromId;
    }
  }
