...
public class VehiclePipoResourceTest {
  ...
  public void testAllActivePullouts() throws Exception {
    ...
    assertTrue(outputJson.contains(" \"service-date\": \"2012-06-15\""));
    assertTrue(outputJson
        .contains("\"pullout-time\": \"2012-06-15T07:23:00-04:00\""));
...
//     System.out.println("BEGIN outputJson");
//     System.out.println(outputJson);
//     System.out.println("END outputJson");
  ...
  public void testAllActiveRealtimePullouts() throws Exception {
    ...
    assertEquals("7777", pulloutList.getPullOuts().getPullOut().get(0)
        .getVehicle().getId());
    assertEquals("7778", pulloutList.getPullOuts().getPullOut().get(1)
        .getVehicle().getId());
    ...
    assertEquals("7777", pulloutList.getPullOuts().getPullOut().get(0)
        .getVehicle().getId());
    assertEquals("7777", pulloutList.getPullOuts().getPullOut().get(1)
        .getVehicle().getId());
    assertEquals("7778", pulloutList.getPullOuts().getPullOut().get(2)
        .getVehicle().getId());
  ...
  public void testActiveRealtimePulloutsByBus() throws Exception {
    ...
    String outputJson = rtResource.getActivePulloutsForBus("7777", null);
    ...
    // Now test with includeAll=true
    outputJson = rtResource.getActivePulloutsForBus("7777", "true");
    ...
    // Now test with explicit includeAll=false
    outputJson = rtResource.getActivePulloutsForBus("7777", "false");
  ...
  public void testActiveRealtimePulloutsByDepot() throws Exception {
    ...
    String outputJson = rtResource.getActivePulloutsForDepot("CSLT", null);
    ...
    // Now test with includeAll=true
    outputJson = rtResource.getActivePulloutsForDepot("CSLT", "true");
    ...
    // Now test with explicit includeAll=false
    outputJson = rtResource.getActivePulloutsForDepot("CSLT", "false");
  ...
  public void testActiveRealtimePulloutsByAgency() throws Exception {
    ...
    // Now test with explicit includeAll=false
    outputJson = rtResource.getActivePulloutsForAgency("MTA BC", "false");
  ...
  private void copyInputFiles(String filename) {
    ...
    try {
      ...
      rtFileIn = this.getClass().getResourceAsStream(
          "pullin-pullout-rt-tcip.json");
      translationFileIn = this.getClass().getResourceAsStream("depot_ids.csv");
      inputFileOut = new FileOutputStream(System.getProperty("java.io.tmpdir")
          + "/UTSPUPUFULL_20120612_1502.txt");
