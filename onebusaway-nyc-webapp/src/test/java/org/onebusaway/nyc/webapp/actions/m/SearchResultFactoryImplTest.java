...
public class SearchResultFactoryImplTest {
  ...
  private StopResult runGetStopResult(List<ServiceAlertBean> serviceAlerts) {
    ...
    when(monitoredStopVisitStructure.getRecordedAtTime()).thenReturn(new Date());  // what???
    ...
    when(_realtimeService.getMonitoredStopVisitsForStop(TEST_STOP_ID, 0, System.currentTimeMillis())).thenReturn(monitoredStopVisits );
