...
public class MtaUtsToTcipVehicleAssignmentConverter {
	...
	/***
	 * Need a class to generate pull out data from the schedule data of an input row.
	 * @param input a MtaUtsVehiclePullInPullOut, which contains both pull in and out scheduled times.
	...
	/***
	 * Need a class to generate pull in data from the schedule data of an input row.
	 * @param input a MtaUtsVehiclePullInPullOut, which contains both pull in and out scheduled times.
	 ...
	 * @return
	 */
	private SCHPullInOutInfo convertToGivenPullInOut (MtaUtsVehiclePullInPullOut inputAssignment, Boolean isPullIn) {
		...
		DateTime pullInTime = getSchedDateAsCalByType(inputAssignment, true);  // Get the pull in time and store it as it's used multiple times.
		DateTime pullOutTime = getSchedDateAsCalByType(inputAssignment, false); // ditto for the pull out time.
		...
		// Set the operator to a new CPTOperatorIden, using the agency id, pass number and operator designator.
		...
		String blockDesignator = inputAssignment.getDepot() + "_" + 
									...
									inputAssignment.getRunNumberField() + "_" + 
									concatPODateTime;
	...
	private DateTime getSchedDateAsCalByType (MtaUtsVehiclePullInPullOut inputAssign, Boolean isPullIn)  {
		DateTime result = null;
		
		String timeStr = isPullIn ? inputAssign.getSchedPI() : inputAssign.getSchedPO();
		
		result = mappingTool.calculatePullInOutDateFromDateUtsSuffixedTime(inputAssign.getServiceDate(), timeStr);
