...
public class BusDepotAssignsToDepotMapTranslator implements
  ...
  public Map<String, List<MtaBusDepotAssignment>> restructure(
    ...
    // For each element, check the depot field of the element against all the existing keys in the map.
    // Can probably check for preexisting depots in a couple ways, including using resultMap.get(depotVal)
    ...
    while (bdIt.hasNext()) {
      ...
      // check the depot field of the element against all the existing keys in the map.
      depotBusAssigns = resultMap.get(bdAssign.getDepot()); // Grab the value for the depot key from the resultMap.
      if (depotBusAssigns == null) { // The depot key does not yet exist in the resultMap.
        depotBusAssigns = new ArrayList<MtaBusDepotAssignment>(); // create a new list for this particular depot.
        depotBusAssigns.add(bdAssign); // add the bus assignment to it.
        resultMap.put(bdAssign.getDepot(), depotBusAssigns); // and stick it into the resultMap.
      } else { // The depot key already exists in the resultMap.
        depotBusAssigns.add(bdAssign); // just add this busassignment to the depotBusAssigns, which already lives in the resultMap.
      }
    }
