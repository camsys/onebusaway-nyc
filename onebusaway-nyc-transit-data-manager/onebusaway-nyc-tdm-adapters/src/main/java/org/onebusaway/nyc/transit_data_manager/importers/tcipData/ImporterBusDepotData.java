...
public class ImporterBusDepotData implements VehicleDepotData {
  ...
  public List<CPTVehicleIden> getVehiclesByDepotNameStr(String depotNameStr) {
    ...
    // I'll do this in two rounds, first to grab all the CPTFleetSubsetGroups that have our depotNameStr
    // (There should really only be one if my other part was right, but I guess I don't want to count on that!)
    ...
    // And second to put all the vehicles in each matching CPTFleetSubsetGroup into the same
    // list for output.
    subGroup = null;
