    ...
    for (TripDetailsBean tripDetailsBean : tripsList) {
      ...
      if(tripStatusBean.getSituations() != null) {
    	  for(ServiceAlertBean serviceAlertBean : tripStatusBean.getSituations()) {
    		  NaturalLanguageStringBean serviceAlert = serviceAlertIdsToNaturalLanguageStringBeans.get(serviceAlertBean.getId());
    	  
    		  if (serviceAlert == null) {
    			  	List<NaturalLanguageStringBean> descriptions = serviceAlertBean.getDescriptions();
    			  	if(descriptions.size() <= 0)
    			  		continue;

    			  	serviceAlertIdsToNaturalLanguageStringBeans.put(serviceAlertBean.getId(), descriptions.get(0));
    		  }
    	  }     
      }
