...
/**
 * A bundle source backed by a Transit Data Manager.
 * 
 * @author jmaki
 *
 */
public class TDMBundleStoreImpl implements BundleStoreService {

	private static Logger _log = LoggerFactory.getLogger(TDMBundleStoreImpl.class);
	...
	private List<BundleItem> getBundleListFromTDM() throws Exception {
		...
		for(JsonObject itemToAdd : bundles) {
			...
			item.setApplicableAgencyIds(applicableAgencyIds);
		...
		_log.info("Found " + output.size() + " bundle(s) available from the TDM.");
	...
	public List<BundleItem> getBundles() throws Exception {
		...
		List<BundleItem> bundlesFromTdm = getBundleListFromTDM();
