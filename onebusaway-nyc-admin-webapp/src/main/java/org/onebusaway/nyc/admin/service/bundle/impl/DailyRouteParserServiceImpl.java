...
import org.onebusaway.nyc.admin.model.ui.*;
 ...
 */
@Component
public class DailyRouteParserServiceImpl implements RouteParserService {
     ...
     */
    @Override
    public List<DataValidationMode> parseRouteReportFile(File fixedRouteReportFile) {
    ...
    public List<DataValidationMode> parseRouteReportInputStream(InputStream fixedRouteReportInputStream, String fixedRouteReportPath) {
        ...
        List<DataValidationMode> parsedModes = new ArrayList<>();
        if (fixedRouteReportInputStream != null) {
            DataValidationMode currentMode = null;
            try {
                BufferedReader in = new BufferedReader(new InputStreamReader(fixedRouteReportInputStream));
                int i = 0;
                for (CSVRecord record : CSVFormat.DEFAULT.parse(in)) {
                    if (i==0) {
                        i++;
                ...
                _log.info("Exception parsing csv file " + fixedRouteReportPath, e);
                e.printStackTrace();
            } catch (IOException e) {
                _log.info("Exception parsing csv file " + fixedRouteReportPath, e);
                e.printStackTrace();
