...
public class GtfsModTask extends BaseModTask implements Runnable {
            ...
            for (GtfsBundle gtfsBundle : gtfsBundles.getBundles()) {
                ...
                if (agencyId != null) {
                    ...
                    if (modUrl != null) {
                        ...
                        logger.log("Transformed " + oldFilename + " to " + newFilename + " according to url " + getModUrl(agencyId));
                    } else {
    ...
    private String getTransform(String agencyId, String path) {
        try {
            return configurationService.getConfigurationValueAsString("admin", agencyId+"_transform").replaceAll(":path", path);
        } catch (Exception e) {}
        return null;
    }
    ...
    private String getModUrl(String agencyId) {
        try {
            return configurationService.getConfigurationValueAsString("admin", agencyId+"_modurl");
        } catch (Exception e) {}
        return null;
    }
