...
public class DiffServiceImpl implements DiffService {
    ...
    public List<String> diff(String filename1, String filename2, InputStream inputStream1, InputStream inputStream2){
        List<String> original = fileToLines(inputStream1);
        List<String> revised  = fileToLines(inputStream2);
        _log.info("Called diff " + this.getClass().getName() +" between (" + filename1 + ") and (" + filename2 + ")");
        List<String> unifiedDiff = DiffUtils.generateUnifiedDiff(
                filename1, filename2, fileToLines(inputStream1), DiffUtils.diff(original, revised), context);
        return unifiedDiff;
    }
