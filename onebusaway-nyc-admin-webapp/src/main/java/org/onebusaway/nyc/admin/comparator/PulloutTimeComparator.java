 ...
 *
 */
public class PulloutTimeComparator implements Comparator<VehicleStatus>{

	private TimeComparator timeComparator;
	private DateTimeFormatter formatter = ISODateTimeFormat.dateTimeNoMillis();
	
	public PulloutTimeComparator(String order) {
		timeComparator = new TimeComparator(order);
	}
	
	@Override
	public int compare(VehicleStatus o1, VehicleStatus o2) {
		if(StringUtils.isBlank(o1.getPulloutTime())) {
			return 1;
		}
		if(StringUtils.isBlank(o2.getPulloutTime())) {
			return -1;
		}
		
		DateTime time1 = formatter.parseDateTime(o1.getPulloutTime());
		DateTime time2 = formatter.parseDateTime(o2.getPulloutTime());
